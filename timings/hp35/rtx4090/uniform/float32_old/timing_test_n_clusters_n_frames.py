...
for component in component_array:
    file_name = "v1.7_timing_" + covar_type + "_" +str(dtype) + "_" + str(component) + ".dat"
    for i, delta in enumerate(deltas):
       ...
       for j in range(n_trials):
           ...
           sgmm = torch_sgmm.ShapeGMMTorch(component,init_cluster_method='chunk',covar_type=covar_type,dtype=dtype,device=device,verbose=False,log_thresh=thresh, max_steps=n_iter)
           sgmm.fit(pos_traj[::delta])
           stop_time = time.process_time()
           elapsed_time.append(stop_time-start_time)
           scores.append(sgmm.predict(pos_traj[::delta])[1])
