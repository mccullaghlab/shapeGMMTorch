...
GAMMA_THRESH = torch.tensor(1E-15,dtype=torch.float64,device="cuda:0")
...
def torch_sgmm_uniform_em(traj_tensor, centers_tensor, vars_tensor, ln_weights_tensor, dtype=torch.float32, device=torch.device("cuda:0"), thresh=1e-3):
    ...
    max_stop.record()
    torch.cuda.synchronize()
    return centers_tensor, vars_tensor, ln_weights_tensor, log_likelihood, em_start.elapsed_time(em_stop), max_start.elapsed_time(max_stop), gamma_start.elapsed_time(gamma_stop)    
    del gamma_tensor
    del gamma_indeces
    torch.cuda.empty_cache()
...
def ln_spherical_gaussian_pdf(disp, var):
    # metadata
    n_atoms = disp.shape[1]
    # compute framewise variance
    mvG = torch.matmul(disp.view(-1,1,n_atoms*3),disp.view(-1,n_atoms*3,1))[:,0,0]
    #mvG = torch.sum(torch.pow(disp,2),(1,2))
    # normalize
    mvG *= -0.5/var
    mvG += -1.5*(n_atoms-1)*torch.log(var)
    return mvG
