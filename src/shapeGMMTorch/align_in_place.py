# library of trajectory alignment protocols using PyTorch and trajectory modifications done in place for memory considerations
...
def align_rot_mats(traj_tensor: torch.Tensor, ref_tensor: torch.Tensor) -> torch.Tensor:
    ...
    # 
    del c_mats, u, s, v, prod_dets
    torch.cuda.empty_cache()
    return rot_mat

@torch.no_grad()
def remove_center_of_geometry_in_place(traj_tensor: torch.Tensor) -> torch.Tensor:
    """
    ...
    traj_tensor.sub_(cog)  # in-place modification
    # clear up memory
    del cog
    if traj_tensor.device.type == "cuda":
        torch.cuda.empty_cache()
...
@torch.no_grad()
def align_uniform_in_place(traj_tensor: torch.Tensor, ref_tensor: torch.Tensor) -> torch.Tensor:
    """
...
@torch.no_grad()
def align_kronecker_in_place(traj_tensor, ref_tensor, precision_tensor):
    """
    Precision weighted Kabsch alignment between trajectory frames and reference

    Modification of traj_tensor is done in place so nothing is returned
    """
...
@torch.no_grad()
def maximum_likelihood_uniform_alignment_in_place(
    ...
    Parameters
    ----------
    traj_tensor : torch.Tensor  ***MODIFIED IN PLACE***
...
@torch.no_grad()
def maximum_likelihood_kronecker_alignment_in_place(
    traj_tensor: torch.Tensor,
    thresh: float = 1e-3,
    max_iter: int = 200,
    verbose: bool = False
...
@torch.no_grad()
def maximum_likelihood_kronecker_alignment_frame_weighted_in_place(
    traj_tensor: torch.Tensor,
    weight_tensor: torch.Tensor,
    ref_tensor: torch.Tensor = None,
